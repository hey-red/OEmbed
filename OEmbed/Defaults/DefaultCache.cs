...
public class DefaultCache : ICache
{
    ...
    public DefaultCache(ICacheKey? cacheKey, ILoggerFactory? loggerFactory, CacheOptions? cacheOptions)
    {
        ...
        _options = cacheOptions ?? new CacheOptions();
    }
        ...
        where T : Base
    {
        ...
        if (item is null)
        {
            using IDisposable mutex = await ScopedMutex.CreateAsync(key);
            // Double check
            item = _cache.Get(key);
            if (item is null)
            {
                ...
                finally
                {
                    ...
                    else
                    {
                        ...
                        _logger.LogDebug(
                            "Saved temporary placeholder object to avoid processing invalid/not found urls.");
                    }
                }
            }
            else
    ...
    public Task<T?> GetAsync<T>(string key) where T : Base
    {
        ...
        return value is T obj ? Task.FromResult<T?>(obj) : Task.FromResult<T?>(null);
    }
